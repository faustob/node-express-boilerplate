...
try {
  ...
  // Keep the app booting and fall back to the already-registered provider.
...
/** Request outcome counter (availability SLI: success vs failure by route). */
...
/** Business-outcome counter for auth flows (register/login/...). */
const authOperations = meter.createCounter('auth.operations', {
  description: 'Authentication operations by name and outcome',
});
